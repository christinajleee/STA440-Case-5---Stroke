...
model1_clean <- tidy(model1_pooled, conf.int = TRUE, exponentiate = FALSE) %>% 
  mutate(
...
model1_clean %>%
  kbl(
    caption = "Pooled Logistic Regression Results — Model 1 (Log-Odds Scale)",
    align = "c"
  ) %>%
...
```{r}
df_mice <- df_mice %>%
  mutate(Age = pmin(pmax(Age, 18), 100))
ggplot(df_mice, aes(x = Age, fill = homeOrRehab)) +
  geom_density(alpha = 0.4) +
  labs(
    title = "Density Plot of Imputed Age by Outcome",
    x = "Age",
    y = "Density",
    fill = "Outcome"
  ) +

...
Implementation We fit a Bayesian hierarchical logistic regression model in JAGS as the following: \[EQUATION\]
...
The posterior distribution for the program indicator had a mean of approximately 0.16, with a 95% credible interval that included 0. As such, this estimate suggests a modest positive association between program implementation and improved discharge outcomes, though the uncertainty interval indicates that the magnitude of this improvement is small and not strongly distinguishable from no effect. In other words, while outcomes trend upward over time, the statistical evidence for a meaningful effect attributable solely to the program is weak.

Several other covariates show consistent relationships with the outcome. Age exhibits a negative association with the probability of home/rehab discharge, indicating older patients have lower odds of receiving a favorable outcome, consistent with both our EDA and clinical expectations. EMS arrivals were associated with worse outcomes, which likely reflects underlying case severity, as patients arriving by EM tend to be more severely impaired.

Finally, we found a substantial site-level heterogeneity - the variance in random intercepts is larger than the magnitude of the program effect, implying that the hospital itself plays a more influential role in discharge outcomes than the intervention program.
